...
-module(galactica_replay_server).
...
get_replay(ReplayUID, WarServerID,RoleID) ->
	send_msg:direct(WarServerID, galactica_replay_server, {get_replay, ReplayUID,WarServerID, RoleID,data_setting:get(server_id)}).
...
handle_info({get_replay, ReplayUID,_WarServerID,RoleID,ServerID},State) ->
	...
	send_msg:direct(ServerID, galactica_replay_server, {get_replay, RoleID,Rep}),
	{noreply,State};
handle_info({get_replay, RoleID,Rep},State) ->
	?unicast(RoleID, #sc_galactica_replay{result=1,fightInfo=[Rep]}),
	{noreply,State};
...
%% ====================================================================
delete_old_replay() ->
    {Time, Day} = data_galactica:get(reply_del_info),
