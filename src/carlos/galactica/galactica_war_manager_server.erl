...
-module(galactica_war_manager_server).
...
send_to_me({new_war, WarID, Team1, Team2}) ->
	{N,IDList} = data_galactica:get(war_servers),
	ID = lists:nth(random:uniform(N), IDList),
	send_msg:direct(ID,galactica_war_manager_server,{new_war_base, WarID, Team1, Team2}).
		...
		carlos ->
			{ok, _} =
				supervisor:start_child(galactica_sup,
									   {galactica_war_sup,
										{galactica_war_sup, start_link, []},
										permanent, infinity, supervisor, [galactica_war_sup]}),
			{ok, _} =
				supervisor:start_child(galactica_sup, 
									   {?MODULE,
										{?MODULE, start_link, []},
...
init([]) ->
    ...
    ets:new(?ETS_GALACTICA_INFO, [named_table, set, public,{keypos,2}]),
%%	init_galactica_pids(),
    {ok, #state{}}.

init_galactica_pids() ->
	List = db_sql:get_galactica_id_list(),
	lists:foreach(fun([WarID]) -> galactica_war_server:start(WarID) end,List).
...
handle_info({new_war, WarID,Attackers,Defenders},State) ->
	galactica_war_server:start([WarID,Attackers,Defenders]),
	{noreply,State};
handle_info({war_stopped, WarID}, State) ->
	ets:delete(?ETS_GALACTICA_INFO,WarID),
	{noreply,State};
handle_info({new_war_base, WarID, Team1, Team2},State) ->
	F = fun()-> 
				P = self(),
				[send_msg:direct(ServerID,galactica_server, {get_player, RoleID, WarID,data_setting:get(server_id),P,attacker})||#member_base_info{serverID=ServerID,roleID=RoleID}<-Team1],
				[send_msg:direct(ServerID,galactica_server, {get_player, RoleID, WarID,data_setting:get(server_id),P,defender})||#member_base_info{serverID=ServerID,roleID=RoleID}<-Team2],
