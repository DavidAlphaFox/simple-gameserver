...
-module(conquerisland_war_manager_server).
...
send_to_me({new_war, WarID, Team1, Team2}) ->
	{N,IDList} = data_conquerisland:get(war_servers),
	ID = lists:nth(random:uniform(N), IDList),
	send_msg:direct(ID,conquerisland_war_manager_server,{new_war_base, WarID, Team1, Team2}).
...
%%删除对应的战场映射
deal_end_war(WarID)->
	ets:delete(?ETS_CONQUERISLAND_WAR_PID_MAP,WarID).

ets_clean()->
	NowSec = util:now(),
	F = fun()->lists:foreach(fun({_,_,_,E,S}=A)->
			if 
				E =< NowSec->
					ets:delete_object(?ETS_CONQUERISLAND_WAR_PID_MAP,A);
				true->
					ignore
			end
		end,ets:tab2list(?ETS_CONQUERISLAND_WAR_PID_MAP)) end, 
	spawn(F).
%% ====================================================================
		...
		carlos ->
			{ok, _} =
				supervisor:start_child(conquerisland_sup,
									   {conquerisland_war_sup,
										{conquerisland_war_sup, start_link, []},
...
init([]) ->
    ...
    ets:new(?ETS_CONQUERISLAND_WAR_PID_MAP, [named_table, set, public,{keypos,2}]),
	% init_carlos_pids(),
...
% 	lists:foreach(fun([WarID]) -> conquerisland_war_server:start(WarID) end,List).
...
handle_info({new_war, WarID,Attackers,Defenders},State) ->
	conquerisland_war_server:start([WarID,Attackers,Defenders]),
	{noreply,State};
...
handle_info({new_war_base, WarID, Team1, Team2},State) ->
	F = fun()-> 
				P = self(),
				[send_msg:direct(ServerID,conquerisland_server, {get_player, RoleID, WarID,data_setting:get(server_id),P,attacker})||#member_base_info{serverID=ServerID,roleID=RoleID}<-Team1],
				[send_msg:direct(ServerID,conquerisland_server, {get_player, RoleID, WarID,data_setting:get(server_id),P,defender})||#member_base_info{serverID=ServerID,roleID=RoleID}<-Team2],
				{Attackers,Defenders} = wait_msg([],[],length(Team1),length(Team2),Team1,Team2),
				% ?INFO("Attackers:~w Defenders:~w ~n",[Attackers,Defenders]),
				new_war(WarID,Attackers,Defenders)
				end,
	Pid = spawn(F),
	% ?ERR("WarID:~w Team1:~w Team2:~w Pid:~w ~n",[WarID,Team1,Team2,Pid]),
	{noreply,State};
