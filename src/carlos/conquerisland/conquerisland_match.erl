...
init([]) ->
	case data_setting:get(server_type) of
		carlos_match ->
			...
			Ets = ets:new(ets_conquerisland_match, []),
...
do_handle_info({match, Team1, Team2, Counter, Level}, #state{ets=Ets, warid=WarID} = State) ->
    ...
    %% 是串行的,不存在这个问题
    % ?ERR("Team1:~w Team2:~w Counter:~w Level:~w ~n",[Team1,Team2,Counter,Level]),
    ...
    case check_match(EtsCounter, Counter) of
        ...
        _ ->
            ...
            case data_conquerisland:get(need_in_room) of
                false ->
                    Msg = {new_war, WarID, parse_request_team2(Members1), parse_request_team2(Members2)},
                    % ?ERR("Msg:~w~n",[Msg]),
                    conquerisland_war_manager_server:send_to_me(Msg);
                _ ->
                    Need = data_conquerisland:get(match_need),
                    mark_in_room([Members1, Members2]),
                    match_room_manager:create_room(?room_type_carlos, {Need, Need}, [Members1, Members2], 
                                                    fun(From, [M1, M2]) ->
...
do_handle_info({request, #request{serverID=ServerID,id=ID,level=Lvl,members=Members}=Request}, #state{ets=Ets, process=Process, warid=WarID} = State) ->
        ...
        Node ->
            case net_adm:ping(Node) of
                ...
                _ ->
                    ...
                    AllLen = 2*data_conquerisland:get(match_need),
                    case get_sign(ID) of
                        _ when AllLen =:= MemberLen andalso Len =:= 1 ->
                            ?INFO("---ai--- request ~w",[Request]),
                            send_msg:direct(ServerID, conquerisland_server, {match_success, ID}),
                            {Members1,Members2} = lists:split(data_conquerisland:get(match_need), Members),
    ...
    %% 给客户端的请求应答
    send_msg:direct(ServerID, conquerisland_server, {sign_result, ID, Result}),
...
do_handle_info({unrequest, ServerID, ID} = Unrequest, #state{ets=Ets} = State) ->
    %%?ERR("unrequest :~p, info:~p~n", [{unrequest, ServerID, ID}, get_sign(ID)]),
    case get_sign(ID) of
        ?undefined ->
            ignore;
        #match_sign_info{is_in_room=IsInRoom, data=Level} = Sign ->
            case IsInRoom of
                ...
                _ ->
                    %% 通知房间
                    match_room_manager:send_unrequest(ID, ?room_type_carlos) 
            end,
            delete_sign(ID)  
        end,
    send_msg:direct(ServerID, conquerisland_server, {reply_unrequest, ID}),
    {noreply, State};
...
%% 数据转移.这里涉及到了3个进程:carlos_match, src、dst,在转移时src已经被锁了,所以不会触发匹配,
%% 因此也不会通知carlos_match进行分配.由于数据的写操作都是在carlos_match里面进行的,而trans_info和
...
do_match(Ets,Team1,Team2,Level) ->
	CountList = get_all_match(Team1,Team2),
	AllMatchList = countList_out_queue_with_sort(Ets,CountList,Level),
	get_match_result(AllMatchList,Team1,Team2).

get_all_match({_,X1,X2,X3,X4,X5},{_,Y1,Y2,Y3,Y4,Y5}) ->
	[{1,X1+Y1},{2,X2+Y2},{3,X3+Y3},{4,X4+Y4},{5,X5+Y5}].
%#request{serverID=ServerID,id=ID,level=Lvl}=Request}
countList_out_queue_with_sort(Ets,CounterList,Level) ->
	countList_out_queue(Ets,CounterList,Level,[]).

countList_out_queue(_,[],_,Res) ->Res;
countList_out_queue(Ets,[{X,0}|T],Level,Res) -> countList_out_queue(Ets,T,Level,[{X,[]}|Res]);
countList_out_queue(Ets,[{X,Need}|T],Level,Res) ->
	...
	{NewList, RequestQ2} = 
		lists:foldl(fun(_, {NewAcc, RequestQAcc}) ->
					...
					end, {[], RequestQ}, lists:seq(1, Need)),
	NewRequestSeq = ?set_lvl_info(RequestSeq,Level,RequestQ2),
	ets:insert(Ets,{{request_queue, X},NewRequestSeq}),
	[{_,CounterSeq}] = ets:lookup(Ets, counter),
	EtsCounter = ?get_lvl_info(CounterSeq, Level),
	NewCounter = ?decf_counter(EtsCounter, X+1, Need),
	ets:insert(Ets, {counter, ?set_lvl_info(CounterSeq, Level, NewCounter)}),
	ets:update_counter(Ets, {total_counter, Level}, -X * Need),
	countList_out_queue(Ets,T,Level,[{X,attr_sort(X,NewList)}|Res]).

attr_sort(1,List) -> 
	lists:sort(fun(#request{members=[#member_base_info{level=Level1}]},#request{members=[#member_base_info{level=Level2}]})-> Level1 > Level2 end, List);
attr_sort(_,List) -> List.

get_match_result(AllMatchList,Team1,Team2) ->
	{T1,T2} = 
		lists:foldl(fun(X,{Match1,Match2}) -> 
							get_match_res(X,Match1,Match2,Team1,Team2)
					end, {[],[]}, AllMatchList),
	#match{team1=T1,team2=T2}.

get_match_res({X,XMatchList},Match1,Match2,Team1,Team2) ->
	M1 = element(X+1,Team1),
	M2 = element(X+1,Team2),
	{_,_,M1L,M2L,_} = 
		lists:foldl(fun(Mat,{M1A,M2A,M1LA,M2LA,SW})->
							RI =  random:uniform(10),
							if RI > 7 andalso M2A > 1 -> {M1A,M2A-1, M1LA,[Mat|M2LA],SW};
							   RI < 2 andalso M1A > 1 -> {M1A-1, M2A,[Mat|M1LA],M2LA,SW};
							   true ->
								   case SW of
									   1 -> if M1A =< 0 -> {M1A,M2A-1,M1LA,[Mat|M2LA],1};
											   true -> {M1A-1,M2A, [Mat|M1LA],M2LA,0}
											end;
									   0 -> if M2A =< 0 -> {M1A-1, M2A, [Mat|M1LA],M2LA,0};
											   true -> {M1A, M2A-1, M1LA, [Mat|M2LA],1}
											end
								   end
							end
					end,{M1,M2,[],[],1},XMatchList),
	{M1L++Match1,M2L++Match2}.
...
do_persist(#state{ets=Ets}=State) ->
    ...
    db_sql:set_etc(?DB_ETC_KEY_CARLOS, [SInfo, EInfo, get()]).
...
request_event(#loop_info{ets=Ets, level=Level} = LoopInfo) ->
    ...
    case Total < 2 * data_conquerisland:get(match_need) of
        true ->
            %% 没有人时,就没必要进行trans
            case Total =:= 0 of
                true ->
        ...
        _ ->
            ...
            case dfs_try_match(Counter) of
                ...
                {Team1, Team2} ->
                    %% 将匹配好的分组发给gen_server
                    % ?ERR("finish match Team1:~w ~n Team2:~w ~n",[Team1,Team2]),
...
dfs_try_match(Counter) ->
    NeedList = data_conquerisland:get(match_need_list),
...
%% 给匹配服发消息(跨服发消息用的接口)
send_to_me(Msg) ->
    send_msg:direct_by_name(carlos_match, conquerisland_match, Msg).
...
trans_info(Level, Prev, Ets) ->
    %%?ERR("-->trans_info:~p,~p.~n",[Level,Prev]),
    lists:foreach(fun(E) ->
                    %% 把后一个层级的队列数据放入上一个层级去
    ...
    lists:foreach(fun(_) ->
                    ...
                    Info = 
                        lists:foldl(fun(_, Acc) ->
                                        [#member_base_info{serverID=1, roleID=0,level=random:uniform(200)}|Acc]
                                    end, [], lists:seq(1, Len)),
                    Request = #request{serverID=1, id=?make_id(TID,Len), members=Info, level=Level},
