...
-record(state, {ets=undefined, process=[], warid=0,type=0}).
...
init([]) ->
	case data_setting:get(server_type) of
		carlos_match ->
			...
			spawn_tick_type(), 
			Ets = ets:new(ets_twins_match, []),
			ets:insert(Ets, {{total_counter, 1}, 0}),
			ets:insert(Ets, {{total_counter, 2}, 0}),
			ets:insert(Ets, {{total_counter, 3}, 0}),
			ets:insert(Ets, {{total_counter, 4}, 0}),
			ets:insert(Ets, {{total_counter, 5}, 0}),
			ets:insert(Ets, {{total_counter, 6}, 0}),
			ets:insert(Ets, {counter, {#counter{},#counter{},#counter{},#counter{},#counter{},#counter{}}}),
			lists:foreach(fun(E) ->
								  ets:insert(Ets, {{request_queue, E}, {queue:new(), queue:new(), queue:new()
			...
			LoopInfo2 = #loop_info{ets=Ets, level=2},
			LoopInfo3 = #loop_info{ets=Ets, level=3},
			LoopInfo4 = #loop_info{ets=Ets, level=4},
			LoopInfo5 = #loop_info{ets=Ets, level=5},
			LoopInfo6 = #loop_info{ets=Ets, level=6},
			{ok, #state{ets=Ets
						, process={ start_loop(LoopInfo1)
									,start_loop(LoopInfo2)
									,start_loop(LoopInfo3)
									,start_loop(LoopInfo4)
									,start_loop(LoopInfo5)
									,start_loop(LoopInfo6)
								  }
						, warid=WarID}};
		_->
			?ERR("not twins match server"),
			ignore
	end.
...
%%%===================================================================
%%% 匹配成功
do_handle_info({match, Team1, Counter, Level}, #state{ets=Ets, warid=WarID,type=Type} = State) ->
	...
	case check_match(EtsCounter, Counter) of
		false ->
			...
			{noreply,State};
		_ ->
			#match{team1=Members} = do_match(Ets, Team1, Level),
            case data_twins:get(need_in_room) of
                false ->
				    Msg = {new_war, WarID, parse_request_team2(Members),Type,Level},
					twins_war_manager_server:send_to_me(Msg);
                _ ->
			        Need = data_twins:get(match_need),
                    mark_in_room([Members]),
			        match_room_manager:create_room(?room_type_twins, {Need}, [Members],
				        						   fun(From, [M]) ->
                                                       send_delete_sign(From, [M]),
						     						   Msg = {new_war, WarID, parse_request_team(M),Type,Level},
												        twins_war_manager_server:send_to_me(Msg)   
										            end) 
            end, 
			{noreply, State#state{warid=WarID + 1}}
	end;

do_handle_info({want, Team1, Counter, Level, {_, RoomID, WantList}}, #state{ets=Ets} = State) ->
    [{_, CounterSeq}] = ets:lookup(Ets, counter),
    EtsCounter = ?get_lvl_info(CounterSeq, Level),
    case check_match(EtsCounter, Counter) of
        false ->
        ...
        _ ->
            #match{team1=Members1, team2=Members2} = do_match(Ets, Team1, Level), 
            match_room_manager:send_to_room(RoomID, {get, WantList, [Members1, Members2]})
    end,
    {noreply, State};

%% 分配请求 
do_handle_info({request, #tw_request{serverID=ServerID,id=ID,level=Level}=Request}
			   , #state{ets=Ets, process=Process} = State) ->
	...
	%% 给客户端的请求应答
	send_msg:direct(ServerID, twins_server, {sign_result, ID, Result}),
	{noreply, State};

%% 重新分配请求 
do_handle_info({re_request, #tw_request{id=ID, level=Level}=Request}, #state{ets=Ets, process=Process} = State) ->
    ...
    case get_sign(ID) of
		...
		#match_sign_info{is_in_room=IsInRoom, data=Level} = Sign ->
            case IsInRoom of
                ...
                _ ->
                    %% 需要通知房间
                    match_room_manager:send_unrequest(ID, ?room_type_twins) 
            end,
            delete_sign(ID) 
	end,
	send_msg:direct(ServerID, twins_server, {reply_unrequest, ID}),
	{noreply, State};
...
%% 数据转移.这里涉及到了3个进程:twins_match, src、dst,在转移时src已经被锁了,所以不会触发匹配,
%% 因此也不会通知twins_match进行分配.由于数据的写操作都是在twins_match里面进行的,而trans_info和
		...
		_ ->
			NewPid =
				case erlang:is_process_alive(Pid) of
					false ->
						start_loop(#loop_info{ets=Ets, level=Level});
					_ ->
						Pid
				end,
...
do_handle_info({tick_type,Type}, State) ->
...
%% do_handle_info(test_set,#state{ets=Ets}=State) ->
%% 	Counter = {counter,{{counter,100,0,0,-1,0},{counter,20,15,-3,-4,-5},{counter,4,1,-5,0,0},{counter,50,0,0,0,0},{counter,0,0,0,0,0},{counter,0,0,0,0,0}}},
...
repair_counter(Ets) ->
	[{_, CounterSeq}] = ets:lookup(Ets, counter),
	NewCounterSeq = 
		lists:foldl(fun(Level,CL) -> 
							Counter = ?get_lvl_info(CounterSeq, Level),
							Counter2 = 
								lists:foldl(fun(N,C)->
													if erlang:element(N+1, C) < 0 ->
														   erlang:setelement(N+1, C, 0);
													   true ->
														   C
													end
											end,Counter,lists:seq(1,5)),
							{counter,V1,V2,V3,V4,V5} = Counter2,
							CT = V1*1+V2*2+V3*3+V4*4+V5*5,
							ets:insert(Ets, {{total_counter, Level}, CT}),
							?set_lvl_info(CL,Level,Counter2)
					end,CounterSeq,lists:seq(1,6)),
	ets:insert(Ets, {counter, NewCounterSeq}).
...
%% 进行分配
do_match(Ets, Team1, Level) ->
	do_match(Team1, Ets, Level, #match{}, #match.team1, 5).
...
check_type(Members,Type)->
	lists:foldl(fun(#tw_request{serverID=ServerID,id=ID,type=RType}, RA)->
						case RType of
							Type ->
								RA andalso true;
							_ ->
								send_msg:direct(ServerID,twins_server,{match_failed, ID}),
								false
						end
				end,true,Members).
...
do_persist(#state{ets=Ets}=State) ->
	...
	db_sql:set_etc(?DB_ETC_KEY_TWINS, [SInfo, EInfo, get()]).
...
loop(#loop_info{lock=true} = LoopInfo) ->
	receive 
		unlock ->
			?MODULE:loop(LoopInfo#loop_info{lock=false})
	end;
%% 进行一次匹配
loop(LoopInfo) ->
	...
	?MODULE:loop(LoopInfo3).

request_event(#loop_info{ets=Ets, level=Level} = LoopInfo) ->
	[{_,Total}] = ets:lookup(Ets, {total_counter, Level}),
	%%     ?ERR("-->request_event:~p,~p,~p~n",[Level,Total,data_twins:get(match_need)]),
	case Total < data_twins:get(match_need) of
		true ->
			%% 没有人时,就没必要进行trans
			case Total =:= 0 of
				true ->
		...
		_ ->
			...
			case dfs_try_match(Counter) of
				undefined ->
					LoopInfo;
				Team1 ->
					erlang:send(?MODULE, {match, statistic([Team1]), statistic([Team1]),Level}), 
					LoopInfo#loop_info{time=util:now()}
			end
	end.
...
want_event(#loop_info{ets=Ets, level=Level}=LoopInfo, {want, RoomID, WantList} = WantData) ->
    ...
    case dfs_try_match(Counter, WantList) of
        ...
        Team1 ->
            %% 将匹配好的分组发给gen_server
            erlang:send(?MODULE, {match_want, statistic([Team1]), statistic([Team1]), Level, WantData})  
    end,
    LoopInfo.
...
dfs_try_match(Counter) ->
	NeedList = data_twins:get(match_need_list),
...
%% 给匹配服发消息(跨服发消息用的接口)
send_to_me(Msg) ->
	send_msg:direct_by_name(twins_match, twins_match, Msg).

%% 将request list转换为member_base_info list
parse_request_team(Team) ->
	lists:foldl(fun(#tw_request{serverID=ServerID, id=ID, members=Info}, Acc) -> 
						send_msg:direct(ServerID, twins_server, {match_success, ID}),
						Info ++ Acc 
				end, [], Team).
...
send_delete_sign(From, Teams) ->
    TeamIDs = 
        lists:foldl(fun(Team, Acc) ->
                        lists:foldl(fun(#tw_request{id=ID}, Acc2) ->
                                        [ID|Acc2]
                                    end, Acc, Team)
                    end, [], Teams),
...
mark_in_room(Teams) ->
    lists:foreach(fun(Team) ->
                    lists:foreach(fun(#tw_request{id=ID}) ->
                                    update_sign(ID, {#match_sign_info.is_in_room, true})
                                end, Team)
                end, Teams).

find_process_level(_, _, 7) ->
	undefined;
...
trans_info(Level, Prev, Ets) ->
	lists:foreach(fun(E) ->
						  ...
						  %% 这里记得更新ID所在的层级
						  lists:foreach(fun(#tw_request{id=ID}) -> update_sign(ID, {#match_sign_info.data, Prev}) end, R ++ F),
...
spawn_tick_type()->
	spawn(fun()-> 
						timer:sleep(1000),
						erlang:register(twins_tick, self()),
						tick_loop(0) end).
%	erlang:send(Pid,{do_tick, erlang:date()}).

tick_loop(LastDate) ->
	Date = erlang:date(),
	erlang:send_after(1000, self(), {do_tick,Date}),
	receive
		{do_tick,LastDate} ->
			?MODULE:tick_loop(LastDate);
		{do_tick, Date} ->
			Type = calc_type(),
			%%?ERR("change_type:~w,~w",[Date,Type]),
			erlang:send(twins_match, {tick_type,Type}),
			?MODULE:tick_loop(Date)
	end.
			
calc_type()->
	calendar:date_to_gregorian_days(erlang:date()) rem 2 + 1.
