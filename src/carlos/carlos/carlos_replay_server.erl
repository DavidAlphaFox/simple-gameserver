...
get_replay(ReplayUID, WarServerID,RoleID) ->
	send_msg:direct(WarServerID, carlos_replay_server, {get_replay, ReplayUID,WarServerID, RoleID,data_setting:get(server_id)}).
...
handle_info({get_replay, ReplayUID,_WarServerID,RoleID,ServerID},State) ->
	...
	send_msg:direct(ServerID, carlos_replay_server, {get_replay, RoleID,Rep}),
	{noreply,State};
handle_info({get_replay, RoleID,Rep},State) ->
	?unicast(RoleID, #sc_carlos_replay{result=1,fightInfo=[Rep]}),
	{noreply,State};
