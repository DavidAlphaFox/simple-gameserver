...
do_handle_info({match, Team1, Team2, Counter, Level}, #state{ets=Ets, warid=WarID} = State) ->
    ...
    case check_match(EtsCounter, Counter) of
        false ->
            {noreply, State};
        _ ->
            CountList = get_all_match(Team1,Team2),    %% 汇总队伍类型需求
            ?INFO("match CountList:~w",[CountList]),
            AllMatchList = countList_out_queue_with_sort(Ets,CountList,Level),     %% 取得实际玩家阵容等级数据，AllMatchList是[{1,X1},{2,X2},{3,X3},{4,X4},{5,X5}],Xn是报名的实际数据[#request{}]
            %% 保存现在能够实现的分配方案,一定要保证AllMatchList是有序的
            NewBalanceOptList = [{lists:sort(AllMatchList),Team1,Team2}|get(?pd_balance_optimize_list)],
            BOLength = erlang:length(NewBalanceOptList),
            MatchBalanceNum = data_carlos:get(match_balance_num),
                ...
                BOLength < MatchBalanceNum ->  % 没有达到优化需要的队伍数量
                    if
                        BOTimestamp =:= 0 -> % 没有启动计时器的话，启动一下
                            erlang:send_after(data_carlos:get(match_balance_time)*1000, self(), bo_tick);
                        true ->
                            ignore
                    end,
                    put(?pd_balance_optimize_list,NewBalanceOptList),
                    NewBOTimestamp = util:now() + data_carlos:get(match_balance_time),
...
do_handle_info({match_want, Team1, Team2, Counter, Level, {_, RoomID, WantList}}, #state{ets=Ets} = State) ->
    ...
    case check_match(EtsCounter, Counter) of
        ...
        _ ->
            {#match{team1=Members1, team2=Members2},_} = do_match(Ets, Team1, Team2, Level), 
...
do_handle_info({request, #request{serverID=ServerID,id=ID,level=Lvl,members=Members}=Request}, #state{ets=Ets, process=Process, warid=WarID} = State) ->
        ...
        Node ->
            case net_adm:ping(Node) of
                ...
                _ ->
                    ...
                    AllLen = 2*data_carlos:get(match_need),
                    case get_sign(ID) of
                        _ when AllLen =:= MemberLen andalso Len =:= 1 ->
                            ?INFO("---ai--- request ~w",[Request]),
                            send_msg:direct(ServerID, carlos_server, {match_success, ID}),
                            {Members1,Members2} = lists:split(data_carlos:get(match_need), Members),
                            Msg = {new_war, WarID, Members1, Members2},
                            war_manager_server:send_to_me(Msg),
                            NewWarID = WarID + 1,
                            Result = 0;
                        ?undefined ->
                            Level = ?get_level(Lvl),
    ...
    %% 给客户端的请求应答
    send_msg:direct(ServerID, carlos_server, {sign_result, ID, Result}),
    {noreply, State#state{warid=NewWarID}};
...
do_handle_info({unrequest, ServerID, ID} = Unrequest, #state{ets=Ets} = State) ->
    ...
    send_msg:direct(ServerID, carlos_server, {reply_unrequest, ID}),
    {noreply, State};
...
%%     do_match(Team2, Ets, Level, Match, #match.team2, 5).
%% 
...
%% 除了match_want外，其他地方不用了。
do_match(Ets,Team1,Team2,Level) ->
	CountList = get_all_match(Team1,Team2),    %% 汇总队伍类型需求
	AllMatchList = countList_out_queue_with_sort(Ets,CountList,Level),     %% 取得实际玩家阵容等级数据
	get_match_result(AllMatchList,Team1,Team2).        %% 根据需求分配为两个队伍
...
countList_out_queue(Ets,[{X,Need}|T],Level,Res) ->
	...
	ets:insert(Ets, {counter, ?set_lvl_info(CounterSeq, Level, NewCounter)}),
	ets:update_counter(Ets, {total_counter, Level}, -X * Need),
	countList_out_queue(Ets,T,Level,[{X,attr_sort(NewList)}|Res]).
...
request_event(#loop_info{ets=Ets, level=Level} = LoopInfo) ->
    ...
    case Total < 2 * data_carlos:get(match_need) of
        true ->
            %% 没有人时,就没必要进行trans
            case Total =:= 0 of
                true ->
...
dfs_try_match(Counter) ->
    NeedList = data_carlos:get(match_need_list),
...
%% 给匹配服发消息(跨服发消息用的接口)
send_to_me(Msg) ->
    send_msg:direct_by_name(carlos_match, carlos_match, Msg).
