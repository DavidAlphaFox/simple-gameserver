...
fix_week_shop(NameString) when erlang:is_list(NameString)->
    case db_sql:search_roleName(util:latin1(NameString)) of
        ?undefined -> not_found;
        RoleId ->
            case role_lib:is_online(RoleId) of
                true ->
                    online;
                false ->
                    Sql1 = io_lib:format(" select distinct roleID from t_pvppoint_consume_2015_11 where roleID = ~w and argID > 52002 and argID < 520011 and time < '2015-11-22 23:59:50';",[RoleId]),
                    Res1 = db_sql:sql_execute_with_log(Sql1),
                    Sql2 = io_lib:format("select distinct roleID from t_unioncoin_consume_2015_11 where roleID = ~w and argID > 40727 and argID < 40732 and time < '2015-11-22 23:59:50';",[RoleId]),
                    Res2 = db_sql:sql_execute_with_log(Sql2),
                    if
                        Res1 =:= {ok,[[RoleId]]}
                          orelse Res2 =:= {ok,[[RoleId]]} ->
                            ShopNumList2 = 
                                lists:foldl(fun(#p_shop_num{sellID=SellID}=E,Acc)-> 
                                        ...
                                        end, [], db_sql:get_shopNumList(RoleId)),
                            db_sql:set_shopNumList(RoleId, ShopNumList2),
                            {ok,NameString};
                        true ->
                            {fail,Res1}
                    end
...
fix_week_shop_all(IsDebug)->
    {ok,List1} = db_sql:sql_execute_with_log("select distinct roleID from t_unioncoin_consume_2015_11 
        where (argID = 40730 or argID = 40731) and time < '2015-11-22 23:59:50'"),
    {ok,Lista} = db_sql:sql_execute_with_log("select roleID,(sum(unioncoin)/2000) from t_unioncoin_consume_2015_11 
        where argID = 40730 and time > '2015-11-23 00:00:10' group by roleID"),
    List2 = [{R,erlang:trunc(N)}||[R,N]<-Lista],
    {ok,Listb} = db_sql:sql_execute_with_log("select roleID,(sum(unioncoin)/8000) from t_unioncoin_consume_2015_11 
        where argID = 40731 and time > '2015-11-23 00:00:10' group by roleID"),
    List3 = [{R,erlang:trunc(N)}||[R,N]<-Listb],
    lists:foldl(fun([RoleId],AccList)-> 
            case role_lib:is_online(RoleId) of
                true ->
                    [{online,RoleId}|AccList];
                false ->
                    BuyFactNum1 = case lists:keysearch(RoleId, 1, List2) of
                                  false ->
                                      0;
                                  {value,{RoleId,BuyNum}} ->
                                      BuyNum
                              end,
                    BuyFactNum2 = case lists:keysearch(RoleId, 1, List3) of
                                  false ->
                                      0;
                                  {value,{RoleId,BuyNum2}} ->
                                      BuyNum2
                              end,
                    OldShopNumList = db_sql:get_shopNumList(RoleId),
                    BuyRecordNum1 = case lists:keysearch(40730, #p_shop_num.sellID, OldShopNumList) of
                                  false ->
                                      0;
                                  {value,ShopNum} ->
                                      ShopNum#p_shop_num.buyNum
                              end,
                    BuyRecordNum2 = case lists:keysearch(40731, #p_shop_num.sellID, OldShopNumList) of
                                  false ->
                                      0;
                                  {value,ShopNum2} ->
                                      ShopNum2#p_shop_num.buyNum
                              end,
                    if
                        BuyRecordNum1 > BuyFactNum1
                          orelse BuyRecordNum2 > BuyFactNum2 ->
                            if
                                IsDebug =:= false ->
                                    ShopNumList2 = 
                                        lists:foldl(fun(#p_shop_num{sellID=SellID}=E,Acc)-> 
