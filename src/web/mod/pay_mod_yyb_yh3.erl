...
-module(pay_mod_yyb_yh3).
-include("common.hrl").

-define(ACCOUNT_TYPE_YYB_YH3, 66).
...
pay_gold(Req) ->
	...
	Method = erlang:list_to_atom(proplists:get_value("method", QueryString)),
	Path = http_uri:decode(proplists:get_value("path", QueryString)),
	QueryString2 = proplists:delete("path", 
									proplists:delete("method", 
													 proplists:delete("billNo",
																	  proplists:delete("appmeta", QueryString)
																	 )
													)
								   ),
	
    case verify_sign(QueryString2, Method, Path) of
        true ->
			...
			case RRType of
				?ACCOUNT_TYPE_YYB_YH3 ->
					Reply = ejson:encode({[{<<"result">>,1},{<<"accid">>,RoleAccID}]}),
					Req:ok({"text/html; charset=utf-8", Reply}),
					
					Amount = erlang:list_to_integer(proplists:get_value("amt", QueryString)),%Amount = get_amount(QueryString),
					QS = mochiweb_util:urlencode(QueryString),
					Sign = "64_"++proplists:get_value("sig", QueryString),
					pay_gold2(RoleID2,Amount,QS,Sign    ,?ACCOUNT_TYPE_YYB_YH3);
				_->
					QS = mochiweb_util:urlencode(QueryString),
		...
		_ ->
			...
			?ERR("yyb3 order fail,sign wrong:~w",[QueryString]),
            Req:ok({"text/html; charset=utf-8", Reply})
    end.
...
verify_sign(QueryString, Method, Path)->
	%?ERR("info:~w,~w,~w",[QueryString,Method,Path]),
    AppKey = <<"QXLgGrIu7a3AaGzl&">>,
