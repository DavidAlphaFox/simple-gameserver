	...
	case GameIdLocal =:= GameId of
		true ->
			...
			if Sign2 =:= Sign ->
				   ...
				   PayAccountID = call_get_wdj_accountID(Ucid),
				   case RoleAccID of
					   PayAccountID ->
						   Reply = ejson:encode({[{<<"result">>,1},{<<"accid">>,RoleAccID}]}),
						   Req:ok({"text/html; charset=utf-8", Reply}),
						   if OrderStatus =:= "S" ->
								  pay_gold2(RoleID2,Amount2,QS,Sign,7);
							  true->
...
call_get_wdj_accountID(Ucid) ->
	AccountServerAddr = data_setting:get(account_server_url),
	URL = lists:flatten(io_lib:format("~s/getWdjID?id=~s", [AccountServerAddr,Ucid])),
