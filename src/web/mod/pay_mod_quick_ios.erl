...
-define(QUICK_IOS_TEST,"9999").       % quick ios test
...
-define(C_SHUN_WANG_IOS, "48").       % 顺网游戏
-define(C_FUJIA,"57").                % 富家
%%quick SDK 渠道定义结束
...
-define(ACCOUNT_TYPE_QUICK_FUJIA,93).             %  富家
%%游戏定义的渠道结束
...
verify_sign(QueryString)->
    Key = "04085067271972718799162711210001",
        ...
        true ->
            ...
            case get_info(Data) of
                {true,RoleID,Amount,OurChannel,OrderNo}->
                    {true,RoleID,Amount,OurChannel,OrderNo,Sign2};
                false ->
                    false
            end;
        false ->
...
quick_channel_to_our_channele(?QUICK_IOS_TEST)-> ?ACCOUNT_TYPE_QUICKSDK;
quick_channel_to_our_channele(?C_SHUN_WANG_IOS)-> ?ACCOUNT_TYPE_SHUNWANG_IOS;
quick_channel_to_our_channele(?C_FUJIA)-> ?ACCOUNT_TYPE_QUICK_FUJIA;
...
do_pay_from_quick(RoleID, Amount, Receipt, Sign, SrcType)->
    case SrcType of
        ...
        ?ACCOUNT_TYPE_QUICKSDK ->
            pay_server:do_pay_from_quick(RoleID,Amount,Receipt,Sign,SrcType);
        ?ACCOUNT_TYPE_SHUNWANG_IOS ->
            pay_server:do_pay_from_shunwang_ios(RoleID,Amount,Receipt,Sign,SrcType);
        ?ACCOUNT_TYPE_QUICK_FUJIA ->
            pay_server:do_pay_from_quick(RoleID,Amount,Receipt,Sign,SrcType);
        _ ->
            pay_server:do_pay_from_quick(RoleID,Amount,Receipt,Sign,SrcType)
    end.