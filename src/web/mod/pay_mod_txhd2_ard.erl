...
-define(ACCOUNT_TYPE_TXHD2_ARD, 71).
...
pay_gold(Req) ->
    ...
    case Md52 =:= Md53 of
        ...
        true ->
            ...
            [{_,_,_,_,GameRole,_}] = xmerl_xpath:string("//extras_params/text()",ParsedDocumentRootElement),
			...
			{RoleAccID,RRType} = db_sql:get_role_accid_and_type(list_to_integer(RoleID)),
			case RRType of
				?ACCOUNT_TYPE_TXHD2_ARD ->
					Reply = ejson:encode({[{<<"result">>,1},{<<"accid">>,RoleAccID}]}),
					Req:ok({"text/html; charset=utf-8", Reply}),
					if Status =:= "0" ->
						   Amount2 = list_to_float(Amount),
						   pay_gold2(list_to_integer(RoleID),trunc(Amount2*10),NTData2,Md52,?ACCOUNT_TYPE_TXHD2_ARD);
					   true->
						   ?ERR("OrderStatus err:~w",[NTData2])
					end ;
				_ ->
					QSL = mochiweb_util:urlencode(QueryStringT),
...
zr_cal_string(NTData) ->
    ZRKey = "65789078675643215436789709878736",
