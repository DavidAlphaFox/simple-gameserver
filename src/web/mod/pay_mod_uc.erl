...
pay_gold(Req) ->
	...
	?DEBUG("Sign:~w, Amount:~w, Ucid:~w, ServerId:~w, callbackInfo:~w\n",[Sign,Amount,Ucid,ServerId,CallbackInfo]),
	...
	GameIdLocal = "541807",
	ServerIdLocal = "3174",
	case GameIdLocal =:= GameId andalso ServerId =:= ServerIdLocal of
		true ->
			Sign2 = sign([Cpid,"amount=",Amount,"callbackInfo=",CallbackInfo,"failedDesc=",FailedDesc,"gameId=",GameId,"orderId=",OrderId,"orderStatus=",OrderStatus,"payWay=",PayWay,"serverId=",ServerId,"ucid=",Ucid,ApiKey]),
			if Sign2 =:= Sign ->
				   RoleAccID = db_sql:get_role_accid(RoleID2),
				   PayAccountID = call_get_uc_accountID(Ucid),
				   case RoleAccID of
					   PayAccountID ->
...
call_get_uc_accountID(Ucid) ->
	AccountServerAddr = data_setting:get(account_server_url),
	URL = lists:flatten(io_lib:format("~s/getUcID?id=~s", [AccountServerAddr,Ucid])),
