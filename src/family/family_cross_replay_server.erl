...
-module(family_cross_replay_server).
...
get_replay(ReplayUID, WarServerID,RoleID) ->
	send_msg:direct(WarServerID, family_cross_replay_server, {get_replay, ReplayUID,WarServerID, RoleID,data_setting:get(server_id)}).
...
start() ->
	{ok, _} =
		supervisor:start_child(family_cross_fight_sup, 
							   {?MODULE,
								{?MODULE, start_link, []},
...
handle_info({get_replay, ReplayUID,_WarServerID,RoleID,ServerID},State) ->
	...
	send_msg:direct(ServerID, family_cross_replay_server, {get_replay, RoleID,Rep}),
	{noreply,State};
handle_info({get_replay, RoleID,Rep},State) ->
	?unicast(RoleID, #sc_familycross_replay{result=1,fightInfo=[Rep]}),
	{noreply,State};
...
handle_info({record_replay, ReplayUID, FightRecord},State) ->
	db_sql:set_fightReplay(ReplayUID, FightRecord,?REPLAY_TYPE_FAMILY_CROSS),
	{noreply,State};
...
%% ====================================================================
delete_old_replay() ->
    {Time, Day} = data_family_cross:get(reply_del_info),
	erlang:send_after(db_manage_server:get_send_after_seconds(Time) * 1000, erlang:self(), delete_yesterday_replay),
    db_sql:del_spec_type_and_time_replay(?REPLAY_TYPE_FAMILY_CROSS, Day).
